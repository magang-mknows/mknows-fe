...
export const UpdateUserProfile = async (payload: TProfilePayload): Promise<TUserDetailResponse> => {
  const { data } = await api.put("/user/profile/me", payload);
  return data;
};

export const UpdateAvatarProfile = async (payload: TAvatarPayload): Promise<TAvatarResponse> => {
