            ...
            <div key={index} className={`${index + 1 === getCurrentNumber ? "block" : "hidden"}`}>
              ...
              {question.answers.map((answer, index) => {
                return (
                  <label
                    key={index}
                    className="relative mb-5  py-3 border-version3-300 border-2 text-sm rounded-lg text-version3-300 cursor-pointer hover:bg-neutral-100 transition-all ease-in-out duration-75 hover:font-bold block "
                    htmlFor={answer.id}
                    id={answer.id}
                  >
                    ...
                    <input
                      ...
                      type="radio"
                      name="choice"
                      className=" h-full w-full peer absolute appearance-none top-0 cursor-pointer"
                    />
                  </label>
                );
              })}
          ...
          {[...Array(questions.length)].map((item, index) => {
            return (
              <div
                ...
                } hover:bg-neutral-100 cursor-pointer h-10 w-10 rounded-md border-[1px] border-version2-400 grid place-items-center`}
              >
                {index + 1}
              </div>
            );
